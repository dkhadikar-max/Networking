...
export default function SwipeCard({ profile, onConnect, onSkip, onSelect }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Compact info — bio/interests are only shown inside the modal */}
      <div className="p-4">
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-bold text-[var(--text)] text-base leading-tight truncate">{name}</h3>
          {location && <span className="text-xs text-[var(--muted)] shrink-0 mt-0.5">{location}</span>}
        </div>
        {headline && <p className="text-sm text-[var(--sub)] mt-0.5 leading-snug line-clamp-1">{headline}</p>}
