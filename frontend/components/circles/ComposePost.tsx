...
const STARTER_TEMPLATES = [
  { label: '🚀 Build', starter: "Building ", tag: 'Building' as CircleTag },
  { label: '🤝 Collab', starter: "Looking for a collaborator or co-founder to build ", tag: 'Collab' as CircleTag },
  { label: '💡 Feedback', starter: "Seeking feedback from builders on ", tag: 'Seeking' as CircleTag },
  { label: '📢 Hiring', starter: "Looking to hire a ", tag: 'Hiring' as CircleTag },
];
...
export default function ComposePost({ onClose, onPosted, groupId, groupName }: Props) {
  ...
  function applyStarter(starter: string, tag: CircleTag) {
    if (!text.trim()) {
      setText(starter);
    } else {
      setText(prev => `${prev} ${starter}`);
    }
  ...
  return (
    ...
    >
      <motion.div
        className="compose-sheet max-w-xl mx-auto"
        initial={{ y: '100%' }}
        animate={{ y: 0 }}
        exit={{ y: '100%' }}
      ...
      >
        ...
        <div className="compose-header">
          <span className="compose-title font-display">
        ...
        <div className="compose-body">
          ...
          <div className="flex items-center gap-1.5 flex-wrap pb-2">
            ...
            {STARTER_TEMPLATES.map(t => (
              <button
                key={t.label}
                type="button"
                className="px-2.5 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-700 hover:bg-teal-50 hover:text-teal-800 border border-slate-200 hover:border-teal-300 transition-all cursor-pointer"
                onClick={() => applyStarter(t.starter, t.tag)}
              >
                {t.label}
              </button>
            ))}
          ...
          <div className="flex items-center justify-between text-xs pt-1">
            <span className="text-[11px] text-slate-400">
              High-signal builder posts get 3x more collaboration requests
            </span>
            <span
          ...
          <div className="compose-section-label">
            Tags <span style={{ color: 'var(--muted)', fontWeight: 400, textTransform: 'none' }}>(pick 1–3)</span>
          </div>
          ...
          {/* Link attachment */}
          <div className="compose-section-label">
            Link <span style={{ color: 'var(--muted)', fontWeight: 400, textTransform: 'none' }}>(optional)</span>
          </div>
          {!linkPreview ? (
          ...
          >
            <svg
              ...
              stroke="currentColor"
              strokeWidth="2.5"
              style={{ transform: showMeta ? 'rotate(180deg)' : 'none', transition: 'transform 0.15s' }}
            >
