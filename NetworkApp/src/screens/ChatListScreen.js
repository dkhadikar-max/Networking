...
function fmtExpiry(hoursLeft) {
  ...
  if (hoursLeft >  0) return `${Math.round(hoursLeft)}h left`;
  return null;
}
...
export default function ChatListScreen({ navigation }) {
  ...
  function renderItem({ item }) {
    ...
    const hasPriority = !!item.hasPriority;
    ...
    return (
      <TouchableOpacity
        ...
        <View style={s.meta}>
          ...
          {!isUnread && isPersist && (
            <View style={s.timerGreen}><Text style={s.timerTxt}>Active</Text></View>
          )}
  ...
  return (
    <View style={s.screen}>
      ...
      {!list.length ? (
        ...
        <FlatList
          data={list}
          keyExtractor={item => item.connection?.id?.toString() || Math.random().toString()}
...
const s = StyleSheet.create({
  ...
  flashBtn:     { width:36, height:36, borderRadius:18, backgroundColor:C.card,
  ...
  row:          { flexDirection:'row', alignItems:'center', paddingHorizontal:16,
                  ...
                  backgroundColor:C.bg },
  avWrap:       { marginRight:12, position:'relative' },
  av:           { width:48, height:48, borderRadius:24 },
  avFallback:   { width:48, height:48, borderRadius:24, backgroundColor:C.primaryLight,
                  justifyContent:'center', alignItems:'center' },
  avInit:       { color:C.primary, fontSize:16, fontWeight:'600' },
  ...
  priorityBadge:{ backgroundColor:C.accentLight||'rgba(244,162,97,0.12)', borderRadius:8,
                  paddingHorizontal:5, paddingVertical:1,
                  borderWidth:1, borderColor:'rgba(244,162,97,0.25)' },
  priorityTxt:  { fontSize:10, color:C.accent },
  ...
  timerGreen:   { backgroundColor:C.primaryLight, borderRadius:10, paddingHorizontal:8,
                  paddingVertical:2, borderWidth:1, borderColor:'rgba(79,138,115,0.25)' },
  timerGray:    { backgroundColor:C.card, borderRadius:10, paddingHorizontal:8,
                  paddingVertical:2, borderWidth:1, borderColor:C.border2 },
